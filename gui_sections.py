...
class GUISections:
    def build_ui(self):
        ...
        # Color Outline Filter Toggle
        self.outline_filter_var = ctk.BooleanVar(value=config.color_outline_filter_enabled)
        self.outline_filter_checkbox = ctk.CTkCheckBox(
            detection_frame,
            text="Color Outline Filter",
        ...
        self.outline_filter_checkbox.grid(row=5, column=0, columnspan=2, sticky="w", padx=6, pady=(8, 0))

        # Outline filter help text
        outline_help = ctk.CTkLabel(detection_frame, text="Only detect players with colored outlines when enabled",
                                text_color="#888", font=("Segoe UI", 10, "italic"))
        outline_help.grid(row=6, column=0, columnspan=3, padx=6, pady=(2, 4), sticky="w")

        # Color Outline HSV Settings Frame (initially hidden)
        self.outline_color_frame = ctk.CTkFrame(detection_frame, fg_color="#1a1a1a")

        # Outline Color Hue Settings
        ctk.CTkLabel(self.outline_color_frame, text="Outline Color (HSV)", text_color="#00e676", font=("Segoe UI", 11, "bold")).grid(row=0, column=0, columnspan=3, pady=(8, 8), padx=6, sticky="w")

        ctk.CTkLabel(self.outline_color_frame, text="Hue Min:", text_color="#fff").grid(row=1, column=0, sticky="w", padx=6)
        self.outline_h_min_slider = ctk.CTkSlider(self.outline_color_frame, from_=0, to=179, command=self.update_outline_h_min)
        self.outline_h_min_slider.set(config.outline_hsv_h_min)
        self.outline_h_min_slider.grid(row=1, column=1, padx=2, sticky="ew")
        self.outline_h_min_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_hsv_h_min), text_color="#ff5e69", width=40)
        self.outline_h_min_value.grid(row=1, column=2, padx=2)

        ctk.CTkLabel(self.outline_color_frame, text="Hue Max:", text_color="#fff").grid(row=2, column=0, sticky="w", padx=6)
        self.outline_h_max_slider = ctk.CTkSlider(self.outline_color_frame, from_=0, to=179, command=self.update_outline_h_max)
        self.outline_h_max_slider.set(config.outline_hsv_h_max)
        self.outline_h_max_slider.grid(row=2, column=1, padx=2, sticky="ew")
        self.outline_h_max_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_hsv_h_max), text_color="#ff5e69", width=40)
        self.outline_h_max_value.grid(row=2, column=2, padx=2)

        ctk.CTkLabel(self.outline_color_frame, text="Sat Min:", text_color="#fff").grid(row=3, column=0, sticky="w", padx=6)
        self.outline_s_min_slider = ctk.CTkSlider(self.outline_color_frame, from_=0, to=255, command=self.update_outline_s_min)
        self.outline_s_min_slider.set(config.outline_hsv_s_min)
        self.outline_s_min_slider.grid(row=3, column=1, padx=2, sticky="ew")
        self.outline_s_min_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_hsv_s_min), text_color="#ff5e69", width=40)
        self.outline_s_min_value.grid(row=3, column=2, padx=2)

        ctk.CTkLabel(self.outline_color_frame, text="Sat Max:", text_color="#fff").grid(row=4, column=0, sticky="w", padx=6)
        self.outline_s_max_slider = ctk.CTkSlider(self.outline_color_frame, from_=0, to=255, command=self.update_outline_s_max)
        self.outline_s_max_slider.set(config.outline_hsv_s_max)
        self.outline_s_max_slider.grid(row=4, column=1, padx=2, sticky="ew")
        self.outline_s_max_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_hsv_s_max), text_color="#ff5e69", width=40)
        self.outline_s_max_value.grid(row=4, column=2, padx=2)

        ctk.CTkLabel(self.outline_color_frame, text="Val Min:", text_color="#fff").grid(row=5, column=0, sticky="w", padx=6)
        self.outline_v_min_slider = ctk.CTkSlider(self.outline_color_frame, from_=0, to=255, command=self.update_outline_v_min)
        self.outline_v_min_slider.set(config.outline_hsv_v_min)
        self.outline_v_min_slider.grid(row=5, column=1, padx=2, sticky="ew")
        self.outline_v_min_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_hsv_v_min), text_color="#ff5e69", width=40)
        self.outline_v_min_value.grid(row=5, column=2, padx=2)

        ctk.CTkLabel(self.outline_color_frame, text="Val Max:", text_color="#fff").grid(row=6, column=0, sticky="w", padx=6)
        self.outline_v_max_slider = ctk.CTkSlider(self.outline_color_frame, from_=0, to=255, command=self.update_outline_v_max)
        self.outline_v_max_slider.set(config.outline_hsv_v_max)
        self.outline_v_max_slider.grid(row=6, column=1, padx=2, sticky="ew")
        self.outline_v_max_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_hsv_v_max), text_color="#ff5e69", width=40)
        self.outline_v_max_value.grid(row=6, column=2, padx=2)

        # Outline Detection Settings
        ctk.CTkLabel(self.outline_color_frame, text="Detection Settings", text_color="#00e676", font=("Segoe UI", 11, "bold")).grid(row=7, column=0, columnspan=3, pady=(12, 8), padx=6, sticky="w")

        ctk.CTkLabel(self.outline_color_frame, text="Radius:", text_color="#fff").grid(row=8, column=0, sticky="w", padx=6)
        self.outline_radius_slider = ctk.CTkSlider(self.outline_color_frame, from_=1, to=10, number_of_steps=9, command=self.update_outline_radius)
        self.outline_radius_slider.set(config.outline_detection_radius)
        self.outline_radius_slider.grid(row=8, column=1, padx=2, sticky="ew")
        self.outline_radius_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_detection_radius), text_color="#ff5e69", width=40)
        self.outline_radius_value.grid(row=8, column=2, padx=2)

        ctk.CTkLabel(self.outline_color_frame, text="Min Pixels:", text_color="#fff").grid(row=9, column=0, sticky="w", padx=6)
        self.outline_min_pixels_slider = ctk.CTkSlider(self.outline_color_frame, from_=1, to=50, number_of_steps=49, command=self.update_outline_min_pixels)
        self.outline_min_pixels_slider.set(config.outline_min_pixels)
        self.outline_min_pixels_slider.grid(row=9, column=1, padx=2, sticky="ew")
        self.outline_min_pixels_value = ctk.CTkLabel(self.outline_color_frame, text=str(config.outline_min_pixels), text_color="#ff5e69", width=40)
        self.outline_min_pixels_value.grid(row=9, column=2, padx=2)

        # Configure column weights
        self.outline_color_frame.grid_columnconfigure(1, weight=1)

        # Position the outline color frame in the grid and initially hide it
        self.outline_color_frame.grid(row=7, column=0, columnspan=3, sticky="ew", padx=6, pady=(8, 15))

        if not config.color_outline_filter_enabled:
            self.outline_color_frame.grid_remove()
        # else:  # Keep it visible if enabled (already positioned above)
